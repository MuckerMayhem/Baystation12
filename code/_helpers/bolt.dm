...
/bolt
	 ...
	 */
/bolt/New(datum/lightning_vector/source, datum/lightning_vector/dest, fade = 50)
	..()
	...
	src.fade    = fade
		 ...
		 */
/bolt/proc/Draw(z, type = /obj/segment, color = "#fff", thickness = 1, split = 0)
 ...
 */
/bolt/proc/Rotate(angle)

	var/line/firstLine = segments[1]
	var/line/lastLine  = segments[segments.len]

	var/datum/lightning_vector/tangent  = vectorSubtract(lastLine.B, firstLine.A)
 ...
 */
/bolt/proc/createBolt(datum/lightning_vector/source, datum/lightning_vector/dest)
	var/list/results = list()

	var/datum/lightning_vector/tangent = vectorSubtract(dest, source)
	var/datum/lightning_vector/normal  = vectorNormalize(new /datum/lightning_vector/(tangent.Y, -tangent.X))
	...
	var/datum/lightning_vector/prevPoint = source
	...
	for(var/i = 2 to positions.len)
		...
		var/datum/lightning_vector/point = new (source.X + (tangent.X * pos) + (normal.X * displacement), source.Y + (tangent.Y * pos) + (normal.Y * displacement))
		point.RoundVector()
 ...
 */
/bolt/proc/GetTurfs(z, accurate = 16)
 ...
 */
/bolt/proc/GetAllTurfs(z, accurate = 16)
	var/list/locs = list()
